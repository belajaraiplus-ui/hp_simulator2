...
fn run_session() -> (String, String, String) {
    ...
    MeasurementEngine::measure_temperature(&mut state, hp_simulator::state::ids::ThermalZoneId::Soc);
    state.time += 0.1;

    VoltageInjection::inject_voltage(
        &mut state,
        hp_simulator::state::ids::RailId::Vbat,
        4.2,
        0.5,
    );
    state.time += 0.5;
    ...
    let reason = status.reason.unwrap_or(
        hp_simulator::session::termination::TerminationReason::VoluntaryStop
    );

    // === CAPTURE OUTPUT ===

    let snapshot = serde_json::to_string(
        &hp_simulator::api::snapshot::build_snapshot(&state)
    ).unwrap();

    let measurements = serde_json::to_string(
        &state.measurements.history
    ).unwrap();

    let ground_truth = serde_json::to_string(
        &GroundTruthBuilder::build(&state, reason)
    ).unwrap();
